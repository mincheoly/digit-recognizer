...
class autoencoder():
    ...
    def encode(self, activations):
        for i in range( len(self.weights) - 1 ):
            activations = sigmoid_vec( np.dot(self.weights[i], activations) + self.biases[i] )
        return activations


    def generate_encoded_dataset(self, data):
        encoded_data = []
        for example in data:
            raw = example[0]
            encoding = self.encode(raw)
            encoded_data.append( (encoding, example[1]) )
        print 'Done generating encoded data'
        return encoded_data

    def test_LC(self, trainExamples, testExamples):
        encoded_training_data = self.generate_encoded_dataset(trainExamples)
        encoded_testing_data = self.generate_encoded_dataset(testExamples)
        LC = linear_classifier(num_features=self.sizes[1])
    ...
    def train(self, training_data, numIters, mini_batch_size, eta, testing_data=None, LC_training_data=None):
        n = len(training_data)
        for iter_pos in range(numIters):
            ...
            if testing_data:
                self.test_LC(LC_training_data, testing_data);
    ...
    def process_mini_batch(self, mini_batch, eta):
        ...
        for feature, label in mini_batch:
            ...
            bias_mini_update, weight_mini_update = self.backpropagate(feature, feature);
            for index in range( len(self.biases) ):
